...
var TimeSeries = function(redis, keyBase, granularities) {
  this.redis = redis;
...
TimeSeries.prototype.recordHit = function(key, timestamp, increment) {
  ...
  Object.keys(this.granularities).forEach(function(gran) {
    var properties = _this.granularities[gran],
      keyTimestamp = getRoundedTime(properties.ttl, timestamp),
      tmpKey = [_this.keyBase, key, gran, keyTimestamp].join(':'),
      hitTimestamp = getRoundedTime(properties.duration, timestamp);

    _this.pendingMulti.hincrby(tmpKey, hitTimestamp, Math.floor(increment || 1));
    _this.pendingMulti.expireat(tmpKey, keyTimestamp + 2 * properties.ttl);
  });
